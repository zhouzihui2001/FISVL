...
def get_dist_launch(args):  # some examples
    ...
    if args.dist == 'f4':
        ...
        return "CUDA_VISIBLE_DEVICES=0,1 WORLD_SIZE=2 /usr/local/miniconda3/bin/python3.8 -W ignore -m torch.distributed.launch --master_port 9999 --nproc_per_node=2 " \
               "--nnodes=1"
...
def run(args):
    if args.task == 'itr_rsicd':
        assert os.path.exists("../../data/RSICD/RSICD_images/")
        args.config = 'configs/Retrieval_rsicd.yaml'
        run_retrieval(args)

    elif args.task == 'itr_rsitmd':
        assert os.path.exists("../../data/RSITMD/images/")
        args.config = 'configs/Retrieval_rsitmd.yaml'
        run_retrieval(args)

    elif args.task == 'itr_ucm':
        assert os.path.exists("../../data/RSITMD/images/")
        args.config = 'configs/Retrieval_ucm.yaml'
        run_retrieval(args)

    elif args.task == 'itr_sydney':
        assert os.path.exists("../../data/RSITMD/images/")
        args.config = 'configs/Retrieval_sydneyg.yaml'
        run_retrieval(args)
