...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, idx_to_group_img=None, image_atts=None, **kwargs):
        # print("-------------x shape 1: {}-------------".format(x.shape)) # [128, 3, 224, 224]
        x = self.patch_embed(x) # [128, 3136, 96]
        # print("-------------x shape 2: {}-------------".format(x.shape))
        if self.ape:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x) # [128, 3136, 96]
        # global
        # x = self.pos_drop(torch.cat([x, conv_g_feat], dim=1))
        # print("-------------x shape 2: {}-------------".format(x.shape))

        for layer in self.layers: # 96->192->384->768->768
            x = layer(x) # [128, 49, 768]
        x = self.norm(x)  # B L C
