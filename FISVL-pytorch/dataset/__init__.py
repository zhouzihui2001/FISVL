...
from dataset.randaugment import RandomAugment, GridMask, GaussianBlur
...
def create_dataset(dataset, config, evaluate=False):
    ...
    train_transform_rsitmd = transforms.Compose([
    ...
    train_transform_rsicd = transforms.Compose([
        transforms.RandomResizedCrop(config['image_res'], scale=(0.5, 1.0),
                                     interpolation=InterpolationMode.BICUBIC),
        transforms.RandomHorizontalFlip(),
        ...
        transforms.RandomApply([GridMask()], p=0.5),
        normalize,
    ])

    train_transform_ucm = transforms.Compose([
        transforms.RandomResizedCrop(config['image_res'], scale=(0.5, 1.0),
                                     interpolation=InterpolationMode.BICUBIC),
        transforms.RandomHorizontalFlip(),
        transforms.RandomApply([GaussianBlur([.1, 2.])], p=0.5),
    ...
    train_transform_sydney = transforms.Compose([
        transforms.RandomResizedCrop(config['image_res'], scale=(0.5, 1.0),
                                     interpolation=InterpolationMode.BICUBIC),
        transforms.RandomHorizontalFlip(),
        transforms.RandomApply([GaussianBlur([.1, 2.])], p=0.5),
    ...
    if dataset == 'pretrain':
    ...
    elif dataset == 're_rsitmd':
        test_dataset = re_eval_dataset(config['test_file'], test_transform, config['image_root'])
        if evaluate:
            return None, None, test_dataset

        train_dataset = re_train_dataset(config['train_file'], train_transform_rsitmd, config['image_root'], config['deletion_rate'])
        val_dataset = re_eval_dataset(config['val_file'], test_transform, config['image_root'])
        return train_dataset, val_dataset, test_dataset
    
    elif dataset == 're_rsicd':
        test_dataset = re_eval_dataset(config['test_file'], test_transform, config['image_root'])
        if evaluate:
            return None, None, test_dataset

        train_dataset = re_train_dataset(config['train_file'], train_transform_rsicd, config['image_root'], config['deletion_rate'])
        val_dataset = re_eval_dataset(config['val_file'], test_transform, config['image_root'])
        return train_dataset, val_dataset, test_dataset
        ...
        train_dataset = re_train_dataset(config['train_file'], train_transform_ucm, config['image_root'], config['deletion_rate'])
        val_dataset = re_eval_dataset(config['val_file'], test_transform, config['image_root'])
        return train_dataset, val_dataset, test_dataset

    elif dataset == 're_sydney':
        test_dataset = re_eval_dataset(config['test_file'], test_transform, config['image_root'])
        if evaluate:
            return None, None, test_dataset

        train_dataset = re_train_dataset(config['train_file'], train_transform_sydney, config['image_root'], config['deletion_rate'])
        val_dataset = re_eval_dataset(config['val_file'], test_transform, config['image_root'])
        return train_dataset, val_dataset, test_dataset
